...
def send_verification(email: str, username: str, code: int):
    body = f'Hey {username},\nThanks for registering an account on Venera! We\'re just here to verify this is you, in your client fill in the code:\n\n{str(code)}'

    # create the email
    audit = multipart.MIMEMultipart()
    audit['From'] = os.getenv('VERIFICATION_EMAIL')
    audit['To'] = email
    audit['Subject'] = 'Verify Email Address for Venera'
